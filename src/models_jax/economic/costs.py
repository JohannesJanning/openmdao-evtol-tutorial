...
def battery_unit_cost(P_bat_s, E_battery_design):
    return P_bat_s * E_battery_design / 1000.0


def energy_cost_model(energy_trip, P_e):
    return energy_trip / 1000.0 * P_e


def navigation_cost_model(MTOM, unitrate, distance_trip_km):
    C_terminal = (MTOM / 1000.0 / 50.0) ** 0.7 * unitrate
    C_enroute = (MTOM / 1000.0 / 50.0) ** 0.5 * distance_trip_km / 100.0 * unitrate
...
def wrap_maintenance_cost(time_trip):
    return 33.0 * time_trip / 3600.0


def battery_maintenance_cost(n_battery_required_annual, P_bat_s, E_battery_design, time_trip, DH, T_D, N_wd):
    return n_battery_required_annual * P_bat_s * E_battery_design / 1000.0 * (time_trip * DH) / (T_D * N_wd)


def maintenance_cost_model(battery_maintenance_cost_val, wrap_maintenance_cost_val):
    return battery_maintenance_cost_val + wrap_maintenance_cost_val
...
def toc_per_seat_min(TOC_flight, N_s, time_trip):
    return TOC_flight / (N_s * time_trip / 60.0)
